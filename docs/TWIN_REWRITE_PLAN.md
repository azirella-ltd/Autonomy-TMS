...
| ↳ **PR-3.B–G** | Lane Transit, Dock Queue, Equipment Flow, Spot-Rate Market, Exception Generator, Intermodal Ramp, Tracking Jitter — same `PhysicsModel` protocol, same feature-flag pattern. Stage as separate PRs in the order specified by §4 of the physics-design doc. | Each unblocks the TRMs listed at its design-doc anchor. |
