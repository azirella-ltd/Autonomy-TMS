...
"Talk to Me" is a persistent AI prompt bar in the TopNavbar that accepts natural language directives from any authenticated user. The system parses directives with an LLM, detects missing information via a smart clarification flow, and routes the completed directive to the appropriate Powell Cascade layer based on the user's role.
...
The provisioning stepper replaces the simple warm-start button with a full 10-step Powell Cascade pipeline. It is stored in `config_provisioning_status` (one row per SC config).

### Steps and Dependencies

```
1. warm_start         → (none)
2. sop_graphsage      → warm_start
3. cfa_optimization   → sop_graphsage
4. execution_tgnn     → warm_start
5. trm_training       → warm_start
6. supply_plan        → cfa_optimization, execution_tgnn
7. decision_seed      → trm_training
8. site_tgnn          → trm_training
9. conformal          → decision_seed
10. briefing          → supply_plan, conformal
```
...
The `ProvisioningStepper.jsx` modal shows a 10-step checklist with:
