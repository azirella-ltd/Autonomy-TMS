...
| **GNN-3** | Build TMS-shape **Lane Volume Forecast tGNN** + training loop + first checkpoint per tenant. **First cut shipped 2026-05-11** ([`backend/app/services/powell/lane_volume_forecast_tgnn.py`](../backend/app/services/powell/lane_volume_forecast_tgnn.py)) — head module + feature catalogues + output dataclass + data-adapter contract + 15 structural tests + 5 torch-gated forward-pass tests. **GNN-3.5 (next):** real ETL on `LaneVolumeForecastDataAdapter` (currently raises `NotImplementedError` per method); pinball-loss training loop wired to `TrainingLoop`; first-checkpoint per tenant. | 2 weeks total — first cut done; GNN-3.5 remaining | **`azirella-powell-core` v0.4.0 shipped 2026-05-11** (TMS pinned at `8c6cd00` via PR #71). |
