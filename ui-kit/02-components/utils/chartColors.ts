 ...
 *
 * Color mapping functions for data visualization in Autonomy.
 * These functions provide consistent color coding across charts and dashboards.
 */

/**
 * Get color for automation percentage values
 * Green scale from light to dark based on performance
 *
 * @param pct - Automation percentage (0-100)
 * @returns Hex color code
 */
export const getAutomationColor = (pct: number) => {
  if (pct >= 80) return "#1b5e20"; // Deep green (80-100%)
  if (pct >= 60) return "#2e7d32"; // Medium green (60-79%)
  if (pct >= 40) return "#43a047"; // Light green (40-59%)
  return "#ef9a9a";                // Light red (0-39%)
};
