...
backup_deployment() {
    ...
    docker save beer_game_backend:latest -o "${BACKUP_DIR}/${BACKUP_NAME}/backend.tar" 2>/dev/null || true
    docker save beer_game_frontend:latest -o "${BACKUP_DIR}/${BACKUP_NAME}/frontend.tar" 2>/dev/null || true

    # Backup database
    log "Backing up database..."
    docker compose exec -T db mysqldump -u root -p${MARIADB_ROOT_PASSWORD} beer_game \
...
rollback() {
    ...
    sleep 10
    docker compose exec -T db mysql -u root -p${MARIADB_ROOT_PASSWORD} beer_game \
