...
class GroupMode(str, Enum):
    """Group operating mode - determines navigation and behavior."""
...
class Group(Base):
    """Organization grouping admins, configs and scenarios (simulations)."""
    ...
    # Group Operating Mode (Learning vs Production)
    mode: Mapped[GroupMode] = mapped_column(
        SAEnum(GroupMode, values_callable=lambda x: [e.value for e in x], name="group_mode_enum"),
        nullable=False,
        server_default=GroupMode.PRODUCTION.value,
        default=GroupMode.PRODUCTION,
    )
    ...
    def is_learning(self) -> bool:
        """Check if group is in learning mode (user education)."""
        return self.mode == GroupMode.LEARNING

    @property
    def is_production(self) -> bool:
        """Check if group is in production mode."""
        return self.mode == GroupMode.PRODUCTION
    ...
    scenarios: Mapped[List["Scenario"]] = relationship("Scenario", back_populates="group", cascade="all, delete-orphan")
    watchlists: Mapped[List["Watchlist"]] = relationship("Watchlist", back_populates="group", cascade="all, delete-orphan")

    # SAP Data Import Cadence System
    sync_job_configs: Mapped[List["SyncJobConfig"]] = relationship(
        "SyncJobConfig", back_populates="group", cascade="all, delete-orphan"
    )

    # Workflow System
    workflow_templates: Mapped[List["WorkflowTemplate"]] = relationship(
        "WorkflowTemplate", back_populates="group", cascade="all, delete-orphan"
    )

    # Planning Cycle Management
    planning_cycles: Mapped[List["PlanningCycle"]] = relationship(
        "PlanningCycle", back_populates="group", cascade="all, delete-orphan"
    )
