    ...
    async def _route_signal(self, signal: SlackSignal) -> None:
        ...
        if "forecast_adjustment" in trm_types and products and sites:
            try:
                ...
                for site_id in sites[:3]:  # Limit to first 3 sites
                    for product_id in products[:5]:  # Limit to first 5 products
                        state = ForecastAdjustmentState(
                            signal_id=f"slack_{signal.id}",
                            product_id=product_id,
                            site_id=site_id,
                            source="slack",
                            signal_type=signal.signal_type,
                            signal_text=signal.signal_summary or signal.message_text[:200],
                            signal_confidence=signal.signal_confidence or 0.5,
                        ...
                        if rec and rec.should_adjust:
                            logger.info(
                                "Slack signal %d -> forecast adjustment: %s %s %.1f%% for %s@%s",
                ...
                logger.warning("TRM routing failed for Slack signal %d: %s", signal.id, e)
