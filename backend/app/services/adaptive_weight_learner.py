...
Persistence:
- Stores learned weights per game/player/config
...
class AdaptiveWeightLearner:
    ...
    def learn_weights(
        ...
        Args:
            ...
            context_id: Game/player/config ID for persistence
    ...
    def get_learned_weights(
        ...
        Args:
            context_id: Game/player/config ID
...
class LearnedWeightConfig(Base):
    """
    Stores learned agent weights per context (game/player/config).
    ...
    context_id = Column(Integer, nullable=False, index=True)  # Game ID, player ID, or config ID
    context_type = Column(String(20), nullable=False, default="game")  # game, player, config
