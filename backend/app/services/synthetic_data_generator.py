...
class SyntheticDataGenerator:
    ...
    async def generate(self, request: GenerationRequest) -> GenerationResult:
        ...
        # 2. Create admin user
        admin_user = await self._create_admin_user(group.id)

        # 3. Create supply chain config
        sc_config = await self._create_supply_chain_config(group.id)
        ...
        await self._create_site_hierarchy(group.id, sites)
        await self._create_product_hierarchy(group.id, items)

        # 8. Create forecasts
        forecasts = await self._create_forecasts(group.id, sites, items)

        # 9. Create inventory levels and policies
        policies = await self._create_inventory_policies(group.id, sites, items)

        # 10. Create planning hierarchy configurations
        await self._create_planning_configs(group.id, sc_config.id)

        # 11. Create agent configurations
        await self._create_agent_configs(group.id, sc_config.id)
    ...
    async def _create_group(self) -> Group:
        """Create the group (organization)."""
        group = Group(
            name=self.request.group_name,
    ...
    async def _create_supply_chain_config(self, group_id: int) -> SupplyChainConfig:
    ...
    async def _create_product_hierarchy(self, group_id: int, items: List[Item]):
    ...
    async def _create_forecasts(self, group_id: int, sites: List[Site], items: List[Item]) -> List[Forecast]:
    ...
    async def _create_inventory_policies(self, group_id: int, sites: List[Site], items: List[Item]) -> List[InvPolicy]:
    ...
    async def _create_planning_configs(self, group_id: int, config_id: int):
    ...
    async def _create_agent_configs(self, group_id: int, config_id: int):
