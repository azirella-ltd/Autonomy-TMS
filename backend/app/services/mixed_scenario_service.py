...
class MixedScenarioService:
    ...
    def create_game(self, game_data: GameCreate, created_by: int = None) -> Game:
        ...
        config_group_id = getattr(config_record, "group_id", None)
        if config_group_id is not None:
            config["group_id"] = config_group_id
    ...
    def update_game(self, game_id: int, payload: Dict[str, Any]) -> Game:
        ...
        if game.group_id is None:
            ...
            if existing_config and getattr(existing_config, "group_id", None) is not None:
                game.group_id = existing_config.group_id
                cfg.setdefault("group_id", existing_config.group_id)
        ...
        if "supply_chain_config_id" in payload:
            ...
            config_group_id = getattr(config_record, "group_id", None)
            if config_group_id is not None:
                game.group_id = config_group_id
                cfg["group_id"] = config_group_id
    ...
    def _start_round_sc_planning(self, game_obj: Game) -> Optional[GameRound]:
        ...
        if not game_obj.group_id:
            logger.error(f"Game {game_obj.id} has no group_id - cannot use SC planning")
            return None
