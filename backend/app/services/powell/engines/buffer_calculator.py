"""
Buffer Calculator - 100% Deterministic

Implements the 4 AWS SC inv_policy types for computing inventory buffers:
- abs_level: Fixed quantity
- doc_dem: Days of coverage (demand-based)
- doc_fcst: Days of coverage (forecast-based)
- sl: Service level with z-score (buffer = z * sigma * sqrt(L))

NOTE: Renamed from SafetyStockCalculator (Feb 2026). The output is an inventory
buffer — an uncertainty absorber — not a hard demand target for MRP. The
underlying AWS SC field names (ss_quantity, ss_policy) remain unchanged for
data model compliance; this rename affects only the service-layer terminology.
...
class PolicyType(Enum):
    """Inventory buffer policy types (AWS SC standard)"""
...
@dataclass
class BufferPolicy:
    """Inventory buffer policy definition"""
    ...
    # Optional overrides
    seasonal_factor: float = 1.0  # Multiply buffer by this during peak seasons
...
@dataclass
class DemandStats:
    """Demand statistics for buffer calculation"""
...
@dataclass
class BufferResult:
    """Inventory buffer calculation result"""
    product_id: str
    location_id: str
    safety_stock: float  # AWS SC field name preserved for data model compliance
    reorder_point: float
    policy_type: PolicyType
    calculation_detail: Dict[str, Any]
    target_inventory: float = 0  # buffer + average demand during lead time
...
@dataclass
class BufferConfig:
    """Configuration for buffer calculator"""
...
class BufferCalculator:
    """
    Inventory buffer calculator implementing 4 policy types.

    100% deterministic - formula-based calculations only.

    NOTE: Computes the buffer component of inventory targets. The buffer is
    an uncertainty absorber — MRP should treat buffer-replenishment planned
    orders at lower priority than demand-driven orders.
    """
    ...
    def compute_safety_stock(
        ...
        stats: Optional[DemandStats] = None
    ) -> BufferResult:
        """
        Compute inventory buffer based on policy type.

        Accepts both positional (product_id, location_id, policy, stats)
        and legacy (stats, policy) signatures for backward compatibility.
        """
        # Handle legacy call signature: compute_safety_stock(stats, policy)
        if isinstance(product_id, DemandStats):
            stats = product_id
            policy = location_id if isinstance(location_id, BufferPolicy) else policy
            product_id = ""
            location_id = ""
        ...
        if stats is None:
            stats = DemandStats(avg_daily_demand=0, std_daily_demand=0, avg_daily_forecast=0)
        ...
        # Reorder point = buffer + (demand during lead time)
        ddlt = stats.avg_daily_demand * stats.lead_time_days
        rop = ss + ddlt

        # Target inventory = buffer + avg demand during (lead time + review period)
    ...
    def _calc_abs_level(self, policy: BufferPolicy) -> tuple:
    ...
    def _calc_doc_dem(self, policy: BufferPolicy, stats: DemandStats) -> tuple:
        ...
        detail = {
            ...
            'avg_daily_demand': stats.avg_daily_demand,
            'formula': 'buffer = avg_daily_demand * days_of_coverage'
        }

        return ss, detail

    def _calc_doc_fcst(self, policy: BufferPolicy, stats: DemandStats) -> tuple:
        ...
        detail = {
            ...
            'formula': 'buffer = avg_daily_forecast * days_of_coverage'
        }

        return ss, detail

    def _calc_service_level(self, policy: BufferPolicy, stats: DemandStats) -> tuple:
        """
        Service level based inventory buffer.

        Formula depends on configuration:

        1. Basic (fixed lead time):
           buffer = z * sigma_D * sqrt(L)

        2. Variable lead time:
           buffer = z * sqrt(L * sigma_D^2 + D^2 * sigma_L^2)

        3. Using forecast error (recommended):
           buffer = z * sigma_FE * sqrt(L)
           Where sigma_FE = forecast error standard deviation
        ...
        if stats.lead_time_std > 0:
            ...
            formula = 'buffer = z * sqrt(L * sigma^2 + D^2 * sigma_L^2)'
        else:
            # Fixed lead time formula: z * sigma * sqrt(L)
            ss = z * sigma * math.sqrt(L)
            formula = 'buffer = z * sigma * sqrt(L)'
    ...
    def compute_batch(
        self,
        items: List[Dict],
        default_policy: Optional[BufferPolicy] = None
    ) -> List[BufferResult]:
        """
        Compute inventory buffer for multiple items.

        Each item dict should have:
        - product_id: str
        - location_id: str
        - policy: BufferPolicy (optional, uses default if not provided)
        - stats: DemandStats
        """
        results = []
        ...
        return [
            {
                ...
                'description': 'Multiplier applied to calculated inventory buffer'
            },
            {
