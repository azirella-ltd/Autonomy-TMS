...
class SyntheticTRMDataGenerator:
        ...
        for site in self.sites:
            ...
            for product_id in self.products:
                ...
                if site.master_type == "MANUFACTURER":
                    base_inv *= 1.5
                elif site.master_type == "MARKET_DEMAND":
                    base_inv *= 0.3
    ...
    async def _generate_daily_forecasts(self):
        ...
        for site in self.sites:
            if site.master_type == "MARKET_SUPPLY":
                continue  # Suppliers don't have demand forecasts
    ...
    async def _generate_daily_orders(self, num_orders: int):
        ...
        for i in range(num_po):
            ...
            supplier_sites = [s for s in self.sites if s.master_type == "MARKET_SUPPLY"]
    ...
    async def _generate_atp_decision(self):
        ...
        site = random.choice([s for s in self.sites if s.master_type != "MARKET_SUPPLY"])
        state = self.site_states[site.id]
