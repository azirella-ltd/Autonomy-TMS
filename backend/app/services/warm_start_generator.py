...
class WarmStartGenerator:
    ...
    def _generate_initial_briefing(self, config_id: int, tenant_id: int) -> None:
        """Ensure briefing schedule exists. Actual briefing content is
        generated by the dedicated `briefing` provisioning step (step 18)
        which runs in an async context. We no longer call
        ExecutiveBriefingService.generate_briefing() from here because
        warm_start runs inside a sync method called from an async
        provisioning pipeline — nesting `asyncio.run()` raises
        ...
        try:
            ...
            self.db.commit()
            logger.info(
                "WarmStart config=%d: briefing schedule ensured for tenant %d "
                "(content will be generated by the briefing step)",
                config_id, tenant_id,
            )
        except Exception as e:
            logger.warning(
                "WarmStart config=%d: briefing schedule setup failed (non-fatal): %s",
                config_id, e,
            )
