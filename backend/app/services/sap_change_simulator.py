...
class SAPChangeSimulator:
    ...
    async def initialize(self) -> Dict[str, Any]:
        ...
        supply_sites = {s["id"] for s in self.state.sites if s["master_type"] in ("VENDOR", "MARKET_SUPPLY")}
    ...
    async def _generate_demand_events(self) -> int:
        ...
        demand_sites = [s for s in self.state.sites if s["master_type"] in ("CUSTOMER", "MARKET_DEMAND")]
    ...
    async def _generate_supply_events(self) -> int:
        ...
        supply_sites = [s for s in self.state.sites if s["master_type"] in ("VENDOR", "MARKET_SUPPLY")]
