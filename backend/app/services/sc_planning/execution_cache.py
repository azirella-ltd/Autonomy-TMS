...
Usage:
    cache = ExecutionCache(db, config_id=2, tenant_id=2)
    await cache.load()
...
class ExecutionCache:
    ...
    async def load(self) -> Dict[str, int]:
        ...
        print(f"  Loading execution cache for config_id={self.config_id}, tenant_id={self.tenant_id}...")
    ...
    async def _load_aggregation_policies(self):
        ...
        result = await self.db.execute(
            select(OrderAggregationPolicy).filter(
                ...
                OrderAggregationPolicy.tenant_id == self.tenant_id,
