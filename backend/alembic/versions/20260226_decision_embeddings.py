...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('site_key', sa.String(100), nullable=True, index=True),
        ...
        # Vector embedding (768 dims, nomic-embed-text)
        sa.Column('embedding', sa.Column('embedding', sa.Text()).type, nullable=True),
    ...
    # Using raw SQL because alembic doesn't know pgvector types
    op.execute("""
        ALTER TABLE decision_embeddings
        ADD COLUMN IF NOT EXISTS embedding vector(768)
    """)
    op.execute("DROP INDEX IF EXISTS idx_decision_embeddings_hnsw")
    op.execute("""
