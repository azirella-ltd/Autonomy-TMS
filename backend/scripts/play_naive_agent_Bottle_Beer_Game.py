"""Run the Naive Agent Showcase scenario for Bottle Beer Game via MixedScenarioService.

- Finds the first scenario whose name contains "Naive Agent Showcase" and whose config
  SC config/template is Bottle Beer Game.
...
TARGET_SC_CONFIG = "bottle beer game"  # case-insensitive match on config/template label


def _find_target_scenario(session) -> Optional[Scenario]:
    scenarios = session.query(Scenario).order_by(Scenario.id).all()
    for g in scenarios:
        if g.name and TARGET_GAME_NAME.lower() in g.name.lower() and "bottle beer game" in g.name.lower():
            return g
    return None
...
def main() -> None:
    ...
    try:
        ...
        if not target:
            print("No Naive Agent Showcase (Bottle Beer Game) scenario found.")
            return
