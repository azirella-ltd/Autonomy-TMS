"""
Script to update AWS SC planning models with group_id foreign keys

This script updates the SQLAlchemy models in aws_sc_planning.py to include
group_id foreign key columns and composite indexes. This matches the database
...
def update_models():
    """Update aws_sc_planning.py models with group_id"""
    ...
    # Tables that need group_id added (all tables with config_id)
    tables_to_update = [
        ('Forecast', 'forecast'),
    ...
    for class_name, table_name in tables_to_update:
        ...
        # Pattern: find line with config_id and add group_id before it
        ...
        if content == old_content:
            print(f"  ⚠️  Already has group_id or pattern not found for {class_name}")
        else:
            print(f"  ✓ Added group_id to {class_name}")
        ...
        # Replace with: Index('idx_TABLENAME_group_config', 'group_id', 'config_id'),\n        Index('idx_TABLENAME_config', 'config_id'),

        config_idx_pattern = rf"(Index\('idx_{table_name}_config', 'config_id'\))"
        config_idx_replacement = rf"Index('idx_{table_name}_group_config', 'group_id', 'config_id'),\n        \1"
    ...
    # Special handling for InvLevel - needs both group_id and config_id added
    print(f"\nUpdating InvLevel...")
    ...
    if inv_level_match:
        ...
        # Add group_id and config_id before snapshot_date
        if 'group_id' not in inv_level_section and 'config_id' not in inv_level_section:
            ...
            print("  ✓ Added group_id and config_id to InvLevel")
        else:
            print("  ⚠️  InvLevel already has group_id or config_id")

    # Special handling for TradingPartner - needs both group_id and config_id added
    ...
    if trading_partner_match:
        ...
        # Add group_id and config_id before created_at
        if 'group_id' not in trading_partner_section and 'config_id' not in trading_partner_section:
            # Find email line (last line before created_at)
            new_trading_partner = re.sub(
                r'(email = Column\(String\(255\)\)\n    website = Column\(String\(255\)\)\n)',
                r'\1    group_id = Column(Integer, ForeignKey("groups.id", ondelete="CASCADE"))\n'
            ...
            print("  ✓ Added group_id and config_id to TradingPartner")
            ...
            print("  ⚠️  TradingPartner already has group_id or config_id")
