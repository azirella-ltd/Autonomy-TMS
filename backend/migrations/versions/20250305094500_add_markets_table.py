...
def upgrade() -> None:
    ...
    if is_sqlite:
        with op.batch_alter_table("market_demands", recreate="always") as batch_op:
    ...
    if is_sqlite:
        # SQLite cannot drop/add constraints post-hoc; table recreated above.
        pass
    else:
        insp = inspect(bind)
...
def downgrade() -> None:
    ...
    if is_sqlite:
        # Best-effort rollback for SQLite without constraint churn.
        op.add_column("market_demands", sa.Column("retailer_id", sa.Integer(), nullable=True))
        op.drop_column("market_demands", "market_id")
    else:
        with op.batch_alter_table("market_demands", schema=None) as batch_op:
