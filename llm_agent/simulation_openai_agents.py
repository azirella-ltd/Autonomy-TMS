"""OpenAI Agent scaffolding for the Beer Game multi-agent hierarchy.
...
The implementation intentionally avoids any Beer Game specific business logic so
...
class OpenAIResponsesAgent:
    ...
    def _format_payload(self, payload: Dict[str, Any]) -> str:
        ...
        return (
            "You are receiving structured state for the Beer Game. "
...
You operate a single Beer Game node (Retailer, Wholesaler, Distributor, or Factory).
...
You are the Beer Game Supervisor. Review node proposals for compliance with
...
GLOBAL_INSTRUCTIONS = """\
You orchestrate the Beer Game network. Analyse the shared state/history to set
...
def resolve_model(preferred: Optional[str] = None, *, custom_gpt: Optional[str] = None) -> str:
    ...
    env_custom = getenv("AUTONOMY_CUSTOM_GPT") or getenv("AUTONOMY_CUSTOM_GPT")
    if env_custom:
        return env_custom
