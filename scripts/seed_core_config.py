...
try:  # pragma: no cover - heavy seed utilities already tested elsewhere
    from backend.scripts.seed_default_tenant import (
        DEFAULT_AGENT_TYPE,
        DEFAULT_CONFIG_NAME,
        DEFAULT_GAME_NAME,
    ...
    logger.warning("Unable to import seed_default_tenant helpers: %s", exc)
    DEFAULT_AGENT_TYPE = "pi_heuristic"
    SEED_UTILS_AVAILABLE = False

    def ensure_ai_agents(*args: Any, **kwargs: Any) -> None:  # type: ignore[misc]
        raise RuntimeError("seed_default_tenant utilities unavailable")
    ...
    def ensure_customer(*args: Any, **kwargs: Any) -> None:  # type: ignore[misc]
        raise RuntimeError("seed_default_tenant utilities unavailable")
